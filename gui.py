...
running = False
video_thread = None
model = YOLO(r'C:\Users\ASUS\Desktop\Interface\Files\Model1.pt') 
...
def update_weight():
    ...
    weight = round(weight+0.0001,3)
    weight_var.config(text=str(weight))
    total_price_var.config(text=str(round(float(weight) * float(price_per_kg_var.cget("text")) + 0.0001,3)))
...
def show_dashboard():
    ...
    img_path = r'C:\Users\ASUS\Desktop\Interface\Files\Logo_of_IIT_Bhilai.png'
